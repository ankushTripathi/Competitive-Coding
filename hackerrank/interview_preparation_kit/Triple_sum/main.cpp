...
long triplets(vector<int> a, vector<int> b, vector<int> c) {
	...
	set<int> q(b.begin(),b.end());
	set<int> r(c.begin(),c.end());
    
    a.assign(p.begin(),p.end());
    b.assign(q.begin(),q.end());
	...
	for(int x : b){
		...
		//long i = distance(p.begin(),p.upper_bound(x));
		//long j = distance(r.begin(),r.upper_bound(x));
//		cout<<x<<" "<<i<<" "<<j<<endl;

        long i = upper_bound(a.begin(),a.end(),x) - a.begin();
        long j = upper_bound(c.begin(),c.end(),x) - c.begin();
		result += i*j;
	}
